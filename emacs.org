...
#+END_SRC
** Bindings
For the most part, I'll try to keep the bindings local to their specific
packages, but a lot of bindings don't really have a package that they are
specific to and as such we'll need to configure them from here.

*** Files
#+BEGIN_SRC emacs-lisp
  (amalthea--leader-key-def
    :keymaps 'normal
    "f d" '(:ignore t :which-key "dotfiles")
    "f d e" '(find-file (expand-file-name (concat "emacs.org" amalthea--emacs-dir)) :which-key "emacs.org")
    "f d c" '(amalthea--tangle-byte-compile-org :which-key "byte compile init.el"))
#+END_SRC
...
everything, it explains more or less anything that you need to know. Functions,
variables, modes, how to say hello in 100+ languages and so on. There's not a
lot of configuration that we'll do here, but we'll do a few keybindings of our
own.
